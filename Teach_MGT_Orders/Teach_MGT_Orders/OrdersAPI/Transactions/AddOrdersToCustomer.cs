...
namespace Teach_MGT_Orders.OrdersAPI.Transactions
{
    ...
    public class AddOrdersToCustomerTxn_Input
    {
        public Customer customer { get; set; }
        public List<Order> orders { get; set; }
    }

    public class AddOrdersToCustomerTxn_InputType : InputObjectType<AddOrdersToCustomerTxn_Input>
    {

    }

    // 2. Create Model: Output type is used for Mutation, it should be included if needed
    public class AddOrdersToCustomerTxn_Output
    {
    ...
    public class AddOrdersToCustomerTxn_OutputType : ObjectType<AddOrdersToCustomerTxn_Output>
    {

    }

    // 4. Transaction - Logic Controller
    public class AddOrdersToCustomerTxn
    {
        public AddOrdersToCustomerTxn()
        {
        }

        public async Task<AddOrdersToCustomerTxn_Output> Execute(AddOrdersToCustomerTxn_Input _input, MVCDbContext _contextFather = null, bool _autoCommit = true)
        {
            AddOrdersToCustomerTxn_Output _output = new AddOrdersToCustomerTxn_Output();
            ...
            try
            {
                ...
                try
                {
                    ...
                    Customer findCustomer = await _contextMGT.Customer.FirstOrDefaultAsync(q => q.CustomerId == _input.customer.CustomerId);

                    if (findCustomer == null)
                    {
                        error = true;
                        _output.resultConfirmation = ResultConfirmation.resultBad(_ResultMessage: "CUSTOMER_NOT_EXISTS", _ResultDetail: _input.customer.CustomerId.ToString()); // If OK                            
                    }
                    else
                    {
                        _input.customer = findCustomer;
                    }

                    if (!error)
                    {
                        //***** 1. Save the Orders (Atomic because is same Context DB)
                        ...
                        AddOrdersToCustomer_Input inheritedInput = new AddOrdersToCustomer_Input { team = inheritedCustomer };
                        AddOrdersToCustomer_Output inheritedOutput = await AddOrdersToCustomerTxn(_input: inheritedInput, _contextFather: _contextMGT, _autoCommit: false);
                        ...
                        AddOrdersToCustomer_Input inheritedInputAlone = new AddOrdersToCustomer_Input { team = inheritedCustomerAlone };
                        AddOrdersToCustomer_Output inheritedOutputAlone = await AddOrdersToCustomerTxn(_input: inheritedInputAlone);
                        */
                        ...
                        _output.resultConfirmation = ResultConfirmation.resultGood(_ResultMessage: "ORDERS_SUCCESSFULLY_SAVED"); // If OK
            ...
            catch (Exception ex) // Main try 
            {
                ...
                _output = new AddOrdersToCustomerTxn_Output(); // Restart variable to avoid returning any already saved data
